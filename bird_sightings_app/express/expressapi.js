...
app.post('/getBirdsByLocation', async (req, res) => {
    ...
    try {
        ...
        if (stateGeometry.type == 'Polygon') {
            const sightingsWithinPolygon = await sightings.aggregate([
                {
                    $match: {
                        location: {
                            $geoWithin: {
            ...
            await Promise.all(coord.map(async polygon => {
                const sightingsWithinPolygon = await sightings.aggregate([
                    {
                        $match: {
                            location: {
        ...
        const birdDetails = await birds.find({ species_code: { $in: uniqueValuesArray } }).toArray();
